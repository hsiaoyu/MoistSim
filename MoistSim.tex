...
\begin{figure*}[h]
  ...
  \caption{The figure shows the time evolution of a plastic armadillo and a plastic bunny as they shrink when exposed to localized heating (red beam in the figure), where redder parts have higher temperature.}
...
\begin{align*}
E_{\mathrm{damp}}(\bv, \bv^{\mathrm{prev}}) &= \frac{\eta}{E}\Delta t \sum_{\bff_{ijk}\in F} \frac{\sqrt{\det \bar\ba_{ijk}}}{2}\Bigg(\frac{h}{4} \left\|\left[\ba_{ijk}^{\mathrm{prev}}\right]^{-1}\frac{\ba_{ijk} - \ba_{ijk}^{\mathrm{prev}}}{\Delta t}\right\|^2_{SV}\\
...
Annuli  & \todo{???} & \todo{???}
\end{tabular}
\caption{Time required to generate a frame of the examples with a step size of \todo{???} s of different degree of freedom(DOF).}
\label{tab:timing}
\end{table}
