...
\item a rest metric, parameterized by the pair of tensor field $\bar\ba, \bar\bb$ over $\Omega$, respectively. These might also evolve over time, due to changes in the shell rest state via growth or shrinkage.
\end{itemize}
...
$$\bar\ba = T^T\bar\ba^{xy}(\bc)T, \qquad  \bar\bb = T^T\bar\bb^{xy}(\bc)T,$$
where
...
\begin{align*}
E_{\mathrm{damp}}(\bv, \bv^{\mathrm{prev}}) &= \eta\Delta t \sum_{\bff_{ijk}\in F} \frac{\sqrt{\det \bar\ba_{ijk}}}{2}\Bigg(\frac{h}{4} \left\|\ba_{ijk}^{-\mathrm{prev}}\frac{\ba_{ijk} - \ba_{ijk}^{\mathrm{prev}}}{\Delta t}\right\|^2_{SV}\\
...
\begin{align}
M\frac{\dot\bv^{i+1}-\dot\bv^{i}}{\Delta t} &= \bF\left(\bv^{i+1},\bv^i\right) \notag\\
...
We discretize equation~\eqref{eq:diffusion} with bilinear Galerkin finite elements on the triangular prisms $F\times[-h/2,h/2]$; The solution $m$ in the prism surrounding face $\bff_{ijk}$ is approximated by a linear combination of basis functions 
...
$$\phi_i^\pm = \frac{(1-u_1-u_2)}{h}\left(\frac{h}{2}\pm z\right); \phi_j^{\pm} = \frac{u_1}{h}\left(\frac{h}{2}\pm z\right); \phi_j^{\pm} = \frac{u_2}{h}\left(\frac{h}{2}\pm z\right).$$
Formulas for the Galerkin mass and stiffness matrices $M_G$ and $K_G$ are derived in Appendix~\ref{XXX}; from these the moisture is updated each time step using implicit time integration,
\begin{equation}
...
When the mechanism for swelling/shrinking is heat rather than moisture, all of the above remains unchanged, with $m$ reinterpreted as temperature rather than moisture.
...
We recommend the usual technique of using the explicit Euler step $\tilde\bv = \bv^{i} + h\dot\bv^i$ as the initial guess in Newton's method each iteration; writing $\bv^{i+1} = \tilde \bv + \Delta t\delta \bv$, we have from Equation~\ref{eq:implicit}
$$g(\delta \bv) = M\delta \bv - \Delta t \bF\left(\tilde \bv + \Delta t\delta \bv, \bv^{i}\right) = 0,$$
which is well-scaled for using Newton's method to solve $g(\delta \bv)=0$. The stiffness and nonlinearity of the elastic forces prohibit very large time steps, though a line search allows time integration using $\Delta t\approx 10^{-4}$ seconds.

The Newton gradient $\nabla g = M + \Delta t^2 \nabla \bF$ is symmetric and almost always positive-definite, and thus amenable to sparse Cholesky decomposition. We use the CHOLMOD solver of the SuiteSparse library~\cite{XXX} for solving the Newton linear system. In the (rare) cases where $\nabla g$ is detected by the solver to be indefinite, we regularize by using $\nabla g = (1+\alpha)M + \Delta t^2 \nabla F$ for progressively larger multiples of $\alpha$, until the Cholesky decomposition succeeds.
\paragraph{Inexact Hessian} The Hessian of the bending energy is expensive to compute, and almost always dominated by the Hessian of the (much stiffer) stretching term; we observed improved performance replacing the exacting bending Hessian with a partial approximation. In particular, we can rewrite the bending energy on face $\bf_{ijk}$ as
\begin{align*}
E_{\mathrm{bending}}(\bv) &= \frac{h^3\sqrt{\det \bar\ba_{ijk}}}{12}\left(r_1(\bv)^2 + r_2(\bv)^2\right)\\
