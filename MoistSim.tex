...
Consider the wrinkling and curling of a drying leaf. The drying process corresponds to water evaporating from the internal cells, so that the tissue contracts in volume. This process typically happens differentially, due to having one side exposed to the sun, or having boundaries farther away from the veins than interior, leading to non-uniform curvature and curling of the leaf ~\cite{Xiao2011, Jeong2013}. Related phenomena are the swelling and wrinkling of paper when exposed to water, such as when coffee is spilled on a notebook page, or the wrinkling and shrinking of plastic when heated. Although the physical mechanisms in these examples are different (shrinking of cells vs swelling of fibers vs contraction of polymers) the \emph{effect} on the objects is the same: the intrinsic geometry of the thin objects is changing over time in response to dynamic changes in the environment. Simulating these common types of deformations requires simulating how the object shape changes in response to changes in its intrinsic geometry, and how the intrinsic geometry changes in response to environmental stimuli. 

In addition to growth, wrinkling, and swelling, other phenomena involving the same physics include burning of more thin objects, especially those made of curved or composite materials; wilting of flowers and dynamic response of plants to like light/moisture/etc; changes and wrinkling in skin due to aging, moisture, creams; shrinking and subsequent wrinkling of clothes; warping of wood due to one-sided heating; etc. There is also increasing excitement about manufacturing processes based on differential or inhomogeneous growth, that require precise control over the material's rest geometry. Exploiting these technologies requires solving challenging inverse problems, with reliable methods for solving \emph{forward} problems an essential first step. Examples include water printers that induces paper bending~\cite{Guberan2012}, two-layer structures consisting of plastic printed on canvas under tension~\cite{Perez2017}, the various works of Nervous Systems~\cite{NervousSystems}, fabrication using networks of filaments that stretch anisotropically when moistened, etc.

Of course, one could capture all of these phenomena by modeling elastic volumes explicitly using tetrahedral or hexahedral elements, and tracking the change in moisture, heat, etc within each element. However, such a volumetric model is computationally expensive, and unnecessary: structures that are thin relative to their surface diameter ought to be able to benefit form a reduced elastic shell model, augmented to track and account for dynamic changes in the environmental stimulus throughout the shell volume. Yet previous methods studying phenomena such as burning of paper~\cite{Liu2009}, drying of leaves~\cite{Jeong2013}, cooking pasta~\cite{Wang2017}, etc, have used either one-off application-specific shell models, or volumetric finite elements, to account for intrinsic geometric changes. Our goal is to fill the gap, and provide a straight-forward low-order numerical implementation for the simulation of thin shells with large deformations, arbitrary rest curvature, realistic material properties, and that supports both isotropic and anisotropic changes in the rest geometry, allowing researchers studying growth and related phenomena to plug in any realistic model and parameter set, and reap a working simulation.
...
$$\bar\bg = \left (\frac{(1 + G)z}{h} + (1 - \frac{z}{h})\right)^2$$
throughout the volume of an XXX by XXX mm slab of thickness $h=XXX$ mm, and evaluate its elastic energy using three methods: (i) tetrahedral finite elements using a St. Venant-Kirchhoff material model, where each tetrahedron is assigned a piecewise constant rest metric by evaluating $\bar\bg$ at its centroid; (ii) our method, as described in section~\ref{sec:discretization}; (iii) our method, using Gingold et al.'s hinge-based shape operator, as discussed in section~\ref{sec:gingold}. We also compute two ground truth energy values: the true elastic energy of a St. Venant-Kirchhoff material integrated over the shell volume, and the energy of the midsurface-based thin limit approximation in equation~\ref{eq:koiter}. 
Figure~\ref{fig:XXX} shows convergence plots of the elastic energy of the finite element and shell methods as a function of mesh resolution; Table~\ref{tab:energy} compares the limit energy values of the discrete methods to the ground truths, for different values of $G$ (the meshes used for this experiment contain XXX tetrahedra/YYY triangles). All finite element energies and simulations were computed using the Vega library~\cite{Vega}.
\begin{table*}
\begin{tabular}{l|ccccccc}
method & $G=0.005$ & $0.03$ & $0.05$ & $0.08$ & $0.1$ & $0.3$ & $0.5$  \\
\hline
St.V-K Energy & $1.6x10^{-6}$ & $5.64x10^{-5}$ & $1.55x10^{-4}$ & $3.87x10^{-4}$ & $5.974x10^{-4}$ & $4.77x10^{-3}$ & $1.2x10^{-2}$ \\
Our Energy & $1.6x10^{-6}$ & $5.77x10^{-5}$ & $1.6x10^{-4}$ & $4.1x10^{-4}$ & $6.4x10^{-4}$ & $5.67x10^{-3}$ & $1.54x10^{-2}$ \\
Gingold Energy & $2.2x10^{-6}$ & $8.01x10^{-5}$ & $2.29x10^{-4}$ & $6.11x10^{-4}$ & $9.83x10^{-4}$ & $1.23x10^{-2}$ & $5.01x10^{-2}$ 
\end{tabular}
\caption{Shell energy computed with St. Venant-Kirchoff model on a thin stab, our discretized energy, and the energy simulated using Gingold's method on high resolution shell.}
\label{tab:energy}
\end{table*}

Our energy follows closely to the St. Venant-Kirhoff energy compared to the method by ~\cite{Gingold04} and converges faster than FEM.
...
Globe & \ref{fig:SwellingDisc} & 5k & 10k & & \\
Annuli  & \todo{???} & \todo{???}
\end{tabular}
...
\section{Validations}
To validate our method, we compare our energy with the energy proposed by Gingold ~\shortcite{Gingold04} which is later adopted in Arcsim ~\cite{Narain:2013:FCA} and the energy of the St. Venant-Kirchoff model of a thin slab of material. Since the method by Gingold does not support growth intrinsically, we compute the rest curvature based on our method and derived the rest angle for the geometry.  

\paragraph{Energy Convergence}

\paragraph{Equilibrium Shape}
Comparisons of the equilibrium shape of the midsurface of the slab with G = 0.03 simulated using FEM, Gingold's method, and our method is given in figure ~\ref{fig:RecGrowth}. As the resolution in the thickness direction increases, FEM gets better approximation for the growth and produces more curved surfaces, whose predicted shape at fine resolution is close to our equilibrium shape. On the other hand, the simulation using Gingold's method generates surface that are more curved than our equilibrium shape.

\begin{figure*}[h]
  \centering
  \includegraphics[width=\textwidth]{Growthtemp.jpg}
  \caption{The top figure shows the gradually curved midsuface of FEM simulation as we increase the resolution. The bottom figure shows the equilibrium shape of three simulations: FEM(top), our(middle), Gingold(bottom).}
  \label{fig:RecGrowth}
\end{figure*} 
\clearpage
