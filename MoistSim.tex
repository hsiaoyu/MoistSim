...
\paragraph{Isotropic Growth} The Riemann mapping theorem guarantees that every surface $M$ with disk topology can be parameterized by the unit disk, with metric conformally equivalent to the Euclidean metric. This insight was exploited by Kim et al~\cite{} to grow a sphere from a square. In the case that $M$ is flat, and this conformal metric is $\ba = e^{2s}I$ for conformal factor $s$, then a disk with prescribed fundamental forms
$$\bar\ba = e^{2s}I, \qquad \bar\bb = 0$$
is expected to grow into $M$ (up to rigid motion), since $M$ is a global minimizer of the elastic energy. We use this fact as a basis for a test: we conformally map a XXX to the unit disk using the method of Crane~\cite{}, assign a rest metric $\bar\ba$ per the above formula, and simulate the resulting growth of the shell. Figure~\ref{XXX} compares the inital shape to the simulation steady state.

When $M$ is not flat, we do not expect the shell to grow exactly into the shape $M$, since embedding the shell as the shape $M$ minimizes stretching energy while ignoring bending energy. Nevertheless, we expect the steady state geometry to closely resemble $M$, especially for small shell thickness $h$. Figure~\ref{XXX} shows the result of attempting to swell the unit disk into a sphere, using the rest fundamental forms from Kim et al~\cite{XXX},
