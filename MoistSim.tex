...
$$m(u,v,z) = \sum_{v\in \{i,j,k\}} \left(m_v^+ \phi_v^+ + m_v^- \phi_v^-\right)$$
parameterized over the prism $\mathcal{T}\times [-h/2,h/2]$ surrounding the canonical unit triangle. In other words
$$\phi_i^\pm = \frac{(1-u-v)}{h}\left(\frac{h}{2}\pm z\right); \phi_j^{\pm} = \frac{u}{h}\left(\frac{h}{2}\pm z\right); \phi_j^{\pm} = \frac{u}{h}\left(\frac{h}{2}\pm z\right).$$
Formulas for the mass and stiffness matrices $M$ and $K$ are derived in Appendix~\ref{XXX}; from these the moisture is updated each time step using implicit time integration,
$$(M/h + DK)\bmm^{i+1} = M(\bmm^i + \bs^i)$$
where $D$ is the diffusion coefficient, and $\bs^i$, the discretized source term, is described below.
...
Integrating the physics~\eqref{eq:implicit} requires some care, as the scale-separation between the stretching and bending forces, along with the very high stiffnesses involved, pose numerical challenges.
\paragraph{Problem Scaling} 
\paragraph{Inexact Hessian}
\paragraph{Inverted Triangles}
